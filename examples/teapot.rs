...
use gaka_rs::rendering::{RenderObject, Renderer};
...
fn main() {
    ...
    let teapot = asset_manager
        .read_string("models/teapot_bezier.txt")
        .unwrap();

    let teapot_surfaces = teapot_to_surfaces(teapot, 10);
    ...
    event_loop.run(move |event, window_target, control_flow| {
        ...
        match event {
            Event::Resumed => {
                ...
                let mut i = 0;
                for mesh in meshes {
                    scene.add_object(&format!("surface{}", i), mesh);
                    i += 1;
                }
...
fn teapot_to_surfaces(teapot_data: String, surface_edges: usize) -> Vec<BezierSurface<4, 4>> {
    let iter = teapot_data.split("\n");
    let points: Vec<Point> = iter
        .map(|point| {
            let point: Vec<f32> = point
                .split(",")
    ...
    let mut surfaces = Vec::with_capacity(points.len() / 16);
    for i in (0..points.len()).step_by(16) {
        let mut grid = [[Point::new(0.0, 0.0, 0.0); 4]; 4];
        for j in 0..4 {
            for k in 0..4 {
                grid[j][k] = points[i + j * 4 + k];
            }
        }
        surfaces.push(BezierSurface::new(grid, surface_edges));
    }
    surfaces
}
